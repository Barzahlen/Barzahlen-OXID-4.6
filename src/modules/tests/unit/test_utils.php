...
class oxTestModules
{
    ...
    private static $_aModuleChain = array();

    /**
     * Initiates the module chain (loads all modules in correct order
     */
    protected static function _initModuleChain()
    {

        $aChain = self::$_aModuleChain;

        foreach( $aChain as $sParent => $aExtends ) {
            $sPrevClass   = $sParent;
            $sPath = array_shift( $aExtends );
            do {
                $sCurrentPath = $sPath;
                self::initClass( dirname( $sCurrentPath ), basename( $sCurrentPath ), $sPrevClass );
                $sPrevClass = basename( $sCurrentPath );
            } while ( $sPath = array_shift( $aExtends ) );
        }
    }

    /**
     * Adds the extended classes to the chain
     *
     * @param string[string] $aExtends assoc array with parent class names as INDEX and extended classes as value
     */
    protected static function _createChain( $aExtends )
    {
        foreach ( $aExtends as $sParentClass => $sExtendedClass ) {
            self::$_aModuleChain[strtolower( $sParentClass )][] = strtolower( $sExtendedClass );
        }     
    }
    ...
    /**
     * Includes and initiates a module class
     * 
     * @param string $sModulePath  module path (relative from the module dir)
     * @param string $sModuleClass module class
     * @param string $sParentClass parent class
     */
    public static function initClass( $sModulePath, $sModuleClass, $sParentClass )
    {
        if ( !class_exists( $sModuleClass ) ) {
            $sBridgeClass = $sModuleClass . "_parent";
            if ( !class_exists( $sBridgeClass, false ) ) {
                eval( "class $sBridgeClass extends $sParentClass {}" );
            }

            $sModuleFile = oxConfig::getInstance()->getConfigParam( "sShopDir" )
                    . "/modules/$sModulePath/" . strtolower( $sModuleClass ) . ".php";
...
class oxTestModuleLoader
{
    ...
    public static function loadModule()
    {
        ...
        if ( count ( self::$_aChain ) ) {
            modConfig::getInstance()->setConfigParam( "aModules", self::$_aChain );
        }
    }
